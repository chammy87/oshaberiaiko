...
// Premium 判定（premiumUntil が過去なら非プレミアム扱い）
function isPremiumFromData(data) {
  ...
  return until.getTime() > Date.now(); // 期限が未来なら有効
}
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const usageDocId = `${userId}_${dayKey}`;
    const usageRef = db.collection("usage_daily").doc(usageDocId);
    ...
    if (!premium) {
      ...
      if (used >= LIMIT) {
        return res.status(429).json({
          ...
          message:
            "無料プランの本日の回数上限に達しました。プレミアムに登録すると制限なく使えます。",
          limit: LIMIT,
          used,
          premium: false,
        });
    ...
    // プロンプトのカンタン合成（必要に応じてテンプレも利用）
    const opener = aikoTemplates?.openers
    ...
    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages,
      max_tokens: premium ? 400 : 220, // 軽め制限
      temperature: 0.8,
    });

    const reply = completion.choices?.[0]?.message?.content?.trim() || "……";

    // 無料ユーザーなら使用回数を +1（JST日単位）
    if (!premium) {
      await usageRef.set(
        {
    ...
    return res.json({
      ...
      usage: premium
        ...
        : {
            premium: false,
            dayKey,
            // 返却時点での使用回数をだいたい示す（正確な used+1 は読み直しが必要）
            note: "本日の残り回数は概算です。",
          },
    });
  } catch (e) {
